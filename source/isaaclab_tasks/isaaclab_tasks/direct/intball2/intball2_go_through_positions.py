...
from isaaclab_tasks.rans import GoThroughPositions3DCfg, GoThroughPositions3DTask, IntBall2Robot, IntBall2RobotCfg


@configclass
class IntBall2GoThroughPositionsEnvCfg(DirectRLEnvCfg):
    # env
    decimation = 4
...
class IntBall2GoThroughPositionsEnv(DirectRLEnv):
    # Workflow: Step
    ...
    cfg: IntBall2GoThroughPositionsEnvCfg
    ...
    def _setup_scene(self):
        ...
        self.task_api = GoThroughPositions3DTask(
            self.scene,
            self.cfg.task_cfg,
            task_uid=0,
            num_envs=self.num_envs,
        ...
        # add intball2 articulation to scene
        self.scene.articulations["intball2"] = self.robot
        # lights
        space_background = False
        if space_background:
            light_cfg = sim_utils.DomeLightCfg(
                intensity=5000.0,
                texture_file=f"{REPO_ROOT_PATH}/assets/textures/your_background_file_name.extension",
            )
        else:
            light_cfg = sim_utils.DomeLightCfg(
                intensity=2500.0,
    ...
    def _get_observations(self) -> dict:
        ...
        # print(task_obs[:5])
        # print(torch.any(torch.isnan(task_obs)))
        return observations
